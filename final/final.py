...
#where the user has the option to leave the game or start it
#or go to the how to play window
def intro():
    win.blit(bg,(0,0))
    intro = True
    GameStart = False
...
# and go back with the go back button
def instructions():
    viewing = True
    back = False
    win.blit(bg,(0,0))
    while viewing:
        ...
        messageOnScreen(win,"five bullets maximum", (0,0,255),150,375,50)
        backButton.draw(win)
        pygame.display.update()
    if back:
        intro()

#this loop is run once when the user leaves the game or loses
#it displays Gameover along with the score and an option to leave
#or start the game from the beginning
def outro():
    outro = True
    while outro:
        for event in pygame.event.get():
...
#this is called if a message is wanted to be displayed on the window
...
#here the gameloop is called where the bots are generated and the user can start
#shooting them also when the levels are multiples of five it takes you to the boss level's
#window where you can start fighting it
def gameloop():
    fightingBoss = False
    num = rocky.level%5
    if num == 0:
